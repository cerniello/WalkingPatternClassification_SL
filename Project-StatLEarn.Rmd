...
# Model
model_RF = randomForest(x = X_train, y = y_train, ntree=101, proximity=T)

# Check if all values are predicted (no NA's)
sum(is.na(model_RF$predicted))
...
# Error on the training set
ETr_LDA = mean(as.vector(pred_LDA$class) != as.vector(y_test))*100
round(ETr_LDA,2)

# Error on the test set
ETe_LDA <- mean(pred_LDA$class != y_test)*100
cat(sprintf("Test error: %f", round(ETe_LDA,2)))
```
...
# Check if all values are predicted (no NA's)
sum(is.na(mod_NB$predicted)) 

# Predictions
pred_NB = predict(mod_NB, X_test)   

# Confusion Matrix
caret::confusionMatrix(as.factor(pred_NB), as.factor(y_test))

# Error on the training set
ETr_NB = mean(as.vector(pred_NB) != as.vector(y_test))*100
round(ETr_NB,2)
