...
```{r knnPred}
knn.fit.predict <- function(X_train, y_train, X_test, y_test, k=7){
  # Model
...
lasso.fit.predict <- function(X_train, y_train, X_test, y_test){
...
svm.fit.predict <- function(X_train, y_train, X_test, y_test){
  # Model
...
```{r stacking}
stacking.model <- function(knn.pred, random.forest.pred, svm.pred){
  ...
  for(i in 1:n){
    p1 <- knn.pred[i]
    p2 <- random.forest.pred[i]
    p3 <- svm.pred[i]
    if(p1!=p2 & p2!=p3 & p1!=p3) {final.pred[i] = knn.pred[i]}
    else {final.pred[i] = getmode(c(p1, p2, p3))}
  }
  return(final.pred)
}
...
```{r stacking2}
stacking.statistics <- function(stacking.pred, y_test){
...
whole.pipeline <- function(Ivana.df, Michele.df, Francesco.df, Francisca.df, Joanna.df){
  ## Dividing into bins
  window.size = 500
  overlap = 200

  Ivana.bins <- slicer(Ivana.df, window.size=window.size, overlap = overlap)
  cat(paste("Ivana", length(Ivana.bins)))
  Michele.bins <- slicer(Michele.df, window.size=window.size, overlap = overlap)
...
whole.pipeline(Ivana.df, Michele.df, Francesco.df, Francisca.df, Joanna.df)
```
